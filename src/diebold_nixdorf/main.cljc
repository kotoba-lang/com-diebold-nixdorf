(ns diebold-nixdorf.main
  "Kotodama WASM entrypoint for the Diebold_nixdorf clean-room actor (L4) — Clojure port.
  ...
  facts (`diebold_nixdorf.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "diebold_nixdorf")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "dieboldn_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "dieboldn_cus"
    :fields [:email :firstName :lastName :phone]            :required [:email :firstName]
    :coerce {}                                  :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "dieboldn_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"diebold_nixdorf.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "diebold_nixdorf-compat" :tier tier :entities entities} 200])
...
(defn start! [] :diebold_nixdorf-compat/ready)
